...
class CampaignController extends Controller
{
    ...
    public function update(Request $request, int $id): JsonResponse
    {
        ...
        return response()->json(['success' => true, 'data' => $this->format($campaign->fresh())]);
    }
    ...
    private function resolve(int $id): Campaign
    {
        ...
        if ($user->isSuperAdmin()) return $campaign;
        if ($user->isClientAdmin() && $campaign->client_id === $user->client_id) return $campaign;
        if ($campaign->user_id === $user->id) return $campaign;
        abort(403);
    }
    ...
    private function authoriseInstance(int $instanceId): void
    {
        ...
        if (!$user->isSuperAdmin() && $instance->client_id !== $user->client_id) abort(403);
    }
